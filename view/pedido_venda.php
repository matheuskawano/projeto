            ...
            <form method="post" id="formDados" name="formDados" action="manutencao/ManutencaoPedVenda.php">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-sm-2">
                        ...
                        <input type="text" class="form-control mdt_hora input-sm" id="dt_hora" name="dt_hora" value="<?= isset($pedVenda['dt_hora']) ? $pedVenda['dt_hora'] : date("d/m/Y H:i:s") ?>"/>
                    </div>
                    <div class="form-group col-sm-7">
                        <label for="cliente">Cliente:</label>
                        <input type="text" class="form-control search" id="cliente" name="cliente" data-table="clientes" value="<?= isset($pedVenda['cliente']) ? $pedVenda['cliente'] : "" ?>">                        
                    </div>                     
                    <div class="form-group col-sm-2">
                        <label for="vl_total">Valor Total:</label>
                        <input type="text" class="form-control valor" id="vl_total" name="vl_total" value="<?= isset($pedVenda['vl_total']) ? $pedVenda['vl_total'] : "" ?>" readonly/>
                    </div>                    
                </div>
                <div class="row">
...
$(document).ready(function() {
	$("#cliente").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url      : "manutencao/ManutencaoCliente.php",
                    method   : "post",
                    dataType : "json",
                    data :{
                        op : "busca",
